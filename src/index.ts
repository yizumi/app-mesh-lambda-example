...
import AppMesh = require("aws-sdk/clients/appmesh");
import {
...
interface IAppMeshGrpcServiceProps {
    /** Name of the AppMesh */
    meshName: string;
...
class AppMeshGrpcService {
    ...
    private async waitForEcsServices(networkConfig: INetworkConfig):Promise<boolean> {
        ...
        const countUnhealthyTasks = async ():Promise<number> => {
            ...
            console.info('Tasks', tasks?.map(t => { return { taskArn: t.taskArn, taskDefArn: t.taskDefinitionArn, lastStatus: t.lastStatus }}));
